...
const createDoors = (scene: TiledMapTest2) => {
  ...
  if (!['%', '.'].includes(scene.sceneInitParams?.adjacentRooms?.north)) {
    const doorNorth = scene.matter.add.sprite(width*.5, height*.1, 'door', '0', {
      isStatic: true,
    });
  ...
  if (!['%', '.'].includes(scene.sceneInitParams?.adjacentRooms?.south)) {
    const doorSouth = scene.matter.add.sprite(width*.5, height*.9, 'door', '0', {
      isStatic: true,
    });
  ...
  if (!['%', '.'].includes(scene.sceneInitParams?.adjacentRooms?.east)) {
    const doorEast = scene.matter.add.sprite(width*.935, height*.5, 'door', '0', {
      isStatic: true,
    });
  ...
  if (!['%', '.'].includes(scene.sceneInitParams?.adjacentRooms?.west)) {
    const doorWest = scene.matter.add.sprite(width*.065, height*.5, 'door', '0', {
      isStatic: true,
    });
